...
class ItemKNNMatcher(ClothesMatcher):
    ...
    def __init__(self):
        ...
        self._item_infos = util.load_item_info(constant.ITEM_FILE, constant.ITEM_IMAGE_PATHS)
    ...
    def find_matched_clothes(self, source_item_id, k):
        """ Given a source item id, find its most matched k items.
            ...
            Return:
                k matched items
        """
        if source_item_id not in self._item_infos:
            print ('No item information stored.')
            return []

        if source_item_id not in self._item_relationship_model:
            print ('Not enough bought history info')
            return []

        # compute how many matched items in each category by category matching model
        cat_id = self._item_infos[source_item_id].get_cat_id()
        if cat_id not in self._cat_relationship_model:
            print ('Failed to find item category information')
            return []
        ...
        print ('Number of matches in each category:', num_matches_in_category)
        print ('Raw matched items:', self._item_relationship_model[source_item_id])
        ...
        for cat_key in num_matches_in_category.keys():
            ...
            for item_key in self._item_relationship_model[source_item_id].keys():
                if item_key in self._item_infos and self._item_infos[item_key].get_cat_id() == cat_key:
                    matched_result_within_category[item_key] = self._item_relationship_model[source_item_id][item_key]
            print ('matched results in each category:', matched_result_within_category)
...
if __name__ == '__main__':
    ...
    #final_result = matcher.find_matched_clothes('1258736', 10)
    final_result = matcher.find_matched_clothes('2730894', 10)
